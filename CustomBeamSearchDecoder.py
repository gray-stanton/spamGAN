...
class BeamSearchDecoderOutput(
    collections.namedtuple("BeamSearchDecoderOutput",
                           ("scores", "predicted_ids", "logits", "parent_ids"))):
  pass


class FinalBeamSearchDecoderOutput(
    collections.namedtuple("FinalBeamDecoderOutput",
                           ["predicted_ids", 'logits', "beam_search_decoder_output"])):
...
class BeamSearchDecoder(decoder.Decoder):
  ...
  def output_size(self):
    ...
    return BeamSearchDecoderOutput(
        ...
        logits=tensor_shape.TensorShape([self._beam_width, self._vocab_size]),
  ...
  def initialize(self, name=None):
    ...
    ### ADDED
    # 300 is just so we don't go over that
  ...
  def finalize(self, outputs, final_state, sequence_lengths):
    ...
    predicted_ids = self._maybe_sort_array_beams(
        outputs.predicted_ids,
        outputs.parent_ids,
        sequence_length=max_sequence_lengths)
    logits = self._maybe_sort_array_beams(
        outputs.logits,
        outputs.parent_ids,
        sequence_length=max_sequence_lengths)
    ...
    outputs = FinalBeamSearchDecoderOutput(
        beam_search_decoder_output=outputs, logits=logits, predicted_ids=predicted_ids)
